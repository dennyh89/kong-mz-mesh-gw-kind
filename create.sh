...
tf -chdir=terraform apply -auto-approve
...
for cluster in ${=CLUSTERS}; do 
  TOKEN=$(curl -s -X  POST   "${KONNECT_SERVER_URL}/v0/mesh/control-planes/$(tf output -state ./terraform/*.tfstate -raw mesh_id)/api/provision-zone"  -H "Authorization:$KONNECT_TOKEN" -d "{\"name\":\"${cluster}\"}" -H "Content-Type: application/json" | jq -r .token)
  ...
  helm repo update
  helm upgrade --install -n kong-mesh-system kong-mesh kong-mesh/kong-mesh -f mesh-global/values.yaml --set-string kuma.controlPlane.zone=${cluster}  --set-string kuma.controlPlane.kdsGlobalAddress=grpcs://${KONNECT_REGION}.mesh.sync.konghq.com:443 --set-string kuma.controlPlane.konnect.cpId=$(tf output -state ./terraform/*.tfstate -raw mesh_id)
done
...
# Sync Gateway routes
DECK_KONNECT_CONTROL_PLANE_NAME="$(tf output -state ./terraform/*.tfstate -raw cp_name )" deck gateway sync gateway/deck.yaml

export LB_IP=$(kubectl --context "kind-${${=CLUSTERS}[1]}" -n kong-gw get svc kong-mesh-ingress-kong-proxy --output jsonpath='{.status.loadBalancer.ingress[0].ip}')
echo "LB_IP=$LB_IP"